...
export default function Checkout(){
 ...
 const [done,setDone]=useState<string|null>(null),[loading,setLoading]=useState(false),[payment,setPayment]=useState<'cod'|'mono'>('cod'),[error,setError]=useState('');
 ...
 const submit=async(e:FormEvent<HTMLFormElement>)=>{
  ...
  const orderPayload={customer:Object.fromEntries(fd),delivery:{provider:'nova-poshta',cityRef:fd.get('cityRef'),warehouseRef:fd.get('warehouseRef')},items:cart.map(x=>({id:x.product.id,name:x.product.name,qty:x.qty,price:x.product.price})),total};
  ...
  if(payment==='mono'){
   ...
   setError(pay.message||pay.error||'Онлайн-оплата поки недоступна.');setLoading(false);return;
  }
 ...
 return <main className="wrap page"><div className="pageIntro"><span className="eyebrow">Оформлення</span><h1>Кошик і доставка</h1></div>{!cart.length?<div className="empty"><h2>Кошик порожній</h2><Link className="primary" href="/catalog">До каталогу</Link></div>:<div className="checkoutGrid">
 <section className="card"><h2>Ваші товари</h2>{cart.map(i=><div className="checkoutLine" key={i.product.id}><div className="mini">{i.product.emoji}</div><div><b>{i.product.name}</b><small>{i.product.brand}</small><span>{money(i.product.price)}</span></div><div className="counter"><button type="button" onClick={()=>setQty(i.product.id,i.qty-1)}><Minus size={14}/></button><b>{i.qty}</b><button type="button" onClick={()=>setQty(i.product.id,i.qty+1)}><Plus size={14}/></button></div><button type="button" className="trash" onClick={()=>remove(i.product.id)}><Trash2 size={17}/></button></div>)}<div className="orderTotal"><span>Разом</span><b>{money(total)}</b></div></section>
 <form className="card form checkoutForm" onSubmit={submit}><h2>Контакти</h2><label>Імʼя<input name="name" required autoComplete="name"/></label><label>Телефон<input name="phone" required inputMode="tel" autoComplete="tel" placeholder="+380…"/></label><label>Email<input name="email" type="email" required autoComplete="email"/></label><div className="checkoutSectionTitle"><Truck size={19}/><h2>Доставка Новою поштою</h2></div><NovaPoshtaPicker/><h2>Оплата</h2><label className="radio"><input type="radio" checked={payment==='cod'} onChange={()=>setPayment('cod')}/> Оплата при отриманні</label><label className="radio"><input type="radio" checked={payment==='mono'} onChange={()=>setPayment('mono')}/> <CreditCard size={17}/> Онлайн через mono</label>{error&&<div className="match">{error}</div>}<button disabled={loading} className="primary wide" type="submit">{loading?'Обробляємо…':payment==='mono'?'Перейти до оплати':'Підтвердити замовлення'}</button></form>
 </div>}</main>
}
