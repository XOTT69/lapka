...
export default function AccountPage(){
 ...
 async function loadPets(uid:string){
  ...
  const {data,error}=await supabase.from('pets').select('id,name,type,age,weight,breed,notes,created_at').eq('user_id',uid).order('created_at',{ascending:true});
 ...
 async function savePet(e:FormEvent){e.preventDefault();if(!editing||!user||savingPet)return;setSaved('');setSavingPet(true);
  ...
  const payload={user_id:user.id,name:editing.name.trim(),type:editing.type,age:editing.age.trim(),weight:editing.weight.trim(),breed:editing.breed.trim(),notes:editing.notes.trim()};
  let error=null;
  if(editing.id){
