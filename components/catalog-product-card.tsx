...
export default function CatalogProductCard({product}:{product:CatalogProduct}){
 ...
 return <article className="productCard">
  <div className="productMedia">
   ...
   {(product.variantCount||0)>1&&<span className="variantBadge">{product.variantCount} варіантів</span>}
  </div>
  <div className="productInfo">
   ...
   <div className="productCardBottom"><div className="productPrice"><strong>{(product.variantCount||0)>1&&product.maxPrice&&product.maxPrice>product.price?'від ':''}{money(product.price)}</strong>{product.oldPrice&&<del>{money(product.oldPrice)}</del>}</div><button className="cartSquare" aria-label="Додати в кошик" onClick={()=>add(catalogToStoreProduct(product))}><ShoppingCart size={18}/></button></div>
  </div>
 </article>
}
